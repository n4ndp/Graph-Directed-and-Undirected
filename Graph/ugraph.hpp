...
class UndirectedGraph : public Graph<TK, TV, TE> {
public:
    ...
    bool insert_edge(TK start, TK end, TE weight) override {
        ...
        it_end->second->edges[it_start->second] = edge;
    ...
    bool remove_edge(TK start, TK end) override {
        ...
        auto edge_it_start = it_start->second->edges.find(it_end->second);
        if (edge_it_start == it_start->second->edges.end()) {
            return false;
        }

        auto edge_it_end = it_end->second->edges.find(it_start->second);
        if (edge_it_end == it_end->second->edges.end()) {
            return false;
        }

        typename Graph<TK, TV, TE>::Edge* edge = edge_it_start->second;

        it_start->second->edges.erase(edge_it_start);

        it_end->second->edges.erase(edge_it_end);
