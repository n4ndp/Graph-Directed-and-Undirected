...
public:
    ...
    bool insert_edge(TI start, TI end, TE weight) override {
        ...
        std::unique_ptr<Edge> new_edge = std::make_unique<Edge>();
        new_edge->vertexes[0] = start_vertex;
        new_edge->vertexes[1] = end_vertex;
        new_edge->weight = weight;

        start_vertex->edges[end_vertex] = std::move(new_edge); // Add edge
    ...
    TE& operator() (TI start, TI end) override {
        if (this->vertexes.find(start) == this->vertexes.end() ||
            this->vertexes.find(end) == this->vertexes.end()) {
            throw std::runtime_error("One or both vertexes don't exist");
        }
    ...
    float density() override {
        ...
        return (float)edge_count / (float)(vertex_count * (vertex_count - 1));
    }
